...
public class SensorDataTable {
    ...
    public SensorDataTable(final Location location, final String name, final SensorType type, final boolean state, final boolean master) {
    ...
    public Sensor toSensor() {
        if (!(getLocation().getBlock().getState() instanceof Sign))
            return null;
        Sensor sensor = null;
        final Sign sign = (Sign) getLocation().getBlock().getState();
        switch (type) {
            case DETECT_ALL:
            ...
            case DETECT_ITEM_AND:
                sensor = new SensorItem(type, sign, name, Arrays.asList(ItemUtils.getItemStringToMatchers(sign.getLine(2), CompassDirection.NO_DIRECTION)));
                break;
            case DETECT_ITEM_OR:
                sensor = new SensorItemOr(type, sign, name, Arrays.asList(ItemUtils.getItemStringToMatchers(sign.getLine(2), CompassDirection.NO_DIRECTION)));
                break;
