...
public class SensorManager {
    ...
    public static Sensor getSensor(final Location loc) {
    ...
    public static Sensor getSensor(final Block loc) {
        return getSensor(loc, false);
    }
    
    public static Sensor getSensor(final Block loc, final boolean checkDatabase) {
        ...
        if (s != null) {
            ...
            if (sensorLock.tryLock()) {
                try {
                    final SensorDataTable data = MinecartManiaSignCommands.instance.getDatabase().find(SensorDataTable.class).where().ieq("x", Integer.toString(loc.getX())).ieq("y", Integer.toString(loc.getY())).ieq("z", Integer.toString(loc.getZ())).ieq("world", loc.getWorld().getName()).findUnique();
                    if (data != null) {
                        s = data.toSensor();
                        if (s != null) {
                            if (!isSign(loc)) {
    ...
    public static void saveSensor(final Sensor sensor) {
    ...
    public static Sensor addSensor(final Location loc, final Sensor s) {
        saveSensor(s);
    ...
    public static boolean delSensor(final Location loc) {
        return sensors.remove(loc) != null;
    }
    
    public static boolean verifySensor(final Sign sign, final Sensor sensor) {
